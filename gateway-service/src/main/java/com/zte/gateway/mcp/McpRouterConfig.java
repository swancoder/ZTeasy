 ...
 * routes: unlike {@link com.zte.gateway.config.GatewayRouteConfig}'s routes, which
 * proxy one request to one response, {@code POST /message} must inject its result
 * into a *different*, already-open connection ({@code GET /sse}) — see
 * {@link McpSessionManager}. Gateway's routing model has no way to express that.
 *
